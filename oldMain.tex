...
In contrast to the previous iteration of the simulation \citep{mufasa}, the black holes in \simba are seeded and grown during the simulation, and the resulting energy from accretion is used to drive feedback, as will be discussed later in this section.  To grow the black holes, \simba uses a two-mode accretion model, in which the torque-limited accretion model is used for cold gas ($T<10^5\,\text{K}$), while for hot gas ($T>10^5\,\text{K}$) Bondi-Hoyle-Littleton accretion is used instead.  For cold gas, the torque-limited accretion rate scales with $\mbh^{1/6}$, similar to \cite{Angles2017}, based on \citet{Hopkins2011}.  While for hot gas, the Bondi accretion rate scales with $\mbh^2$, based on the standard \cite{Bondi1952} formula.


Since any galaxy can contain more than one supermassive black hole (e.g. due to mergers), the primary (`central') black hole for which the Eddington fraction is computed is defined as the most massive black hole in the galaxy, which at every time step is repositioned at the location of the global minimum of its host galaxy's potential well based on the assumption of efficient dynamical friction \citep{simba}.  If more than two black holes are located within a single black hole kernel size, then they are allowed to merge instantaneously if their relative velocity is lower than three times their mutual escape velocity \citep{simba}.  
...
After running the \simba \citep{simba} simulation, \salad \citep{salad} is employed in post-processing to identify all available galaxies.  Before requiring any selection criteria, our original sample has 20,802 galaxies at $z=0$, 13,190 at $z=1$, and 8,699 at $z=2$.  \red{By requiring all galaxies to have a non-zero number of gas particles, at $z=0$ we now have  2,733 ($\sim 13\%$) galaxies, at $z=1$ we now have 4,595 galaxies ($\sim 35\%$) and at $z=2$ we now have 5,945 galaxies ($\sim 68\%$).}
\begin{figure*}[htb!]
    \centering
...
To focus our study on massive quenched galaxies comparable to current observational surveys \citep[e.g.][]{rowlands, french_2015, french_2017, French_2018, suessSquiggle, Bezanson_2022, Baron2023, french23, Spilker2025}, we impose a minimum mass requirement.  First, to restrict our sample to the massive galaxies seen by observations, we first require that all galaxies in our sample have $M_*>10^{10}M_\odot$.  \red{From our initial sample of galaxies with a non-zero amount of gas particles, we find that the requirement that $M_*>10^{10}M_\odot$ is only satisfied for $\sim 7\%$ (191/2733) of galaxies at $z=0$.  This then drops to $\sim 2.3\%$ (105/4595) of galaxies at $z=1$, and drops again to $\sim 0.9\%$ (54/5945) at $z=2$.}

Secondly, we will also require that the galaxies in our sample have enough molecular $(\hh$) gas to be detectable via CO by current observational efforts such as ALMA.  While this so-called `detectability threshold' will be heavily dependent on redshift \citep{Bezanson_2022, Baron2023}, we find in the intermediate-redshift regime that a conservative lower mass limit for molecular gas is at $\mhh=10^9M_\odot$ \citep{suess_2017, suessSquiggle}.  Combining this with our massive galaxy requirement results in our `high-mass' requirement of:
\begin{equation}
    % \mhh>10^9M_\odot\ \&\ M_*>10^{10}M_\odot\quad \text{(Mass Limit)}
    \text{`H2 Rich':}\quad \mhh>10^9M_\odot
    \label{eq:massLimit}
\end{equation}
\red{Now, from our initial sample of galaxies with a non-zero number of gas particles and $M_*>10^{10}M_\odot$, we find that $\sim 70.2\%$ (134/191) of galaxies at $z=0$ are gas rich based on \eq\ref{eq:massLimit}.  This increases to $\sim 81.9\%$ (86/105) of galaxies at $z=1$ which are gas-rich, and $\sim 92.6\%$ (50/54) of galaxies at $z=2.$}
\begin{equation}
    \text{`H2 Poor':}\quad \mhh\leq 10^9M_\odot
    \label{eq:massLowLim}
\end{equation}
\red{Now, from our initial sample of galaxies with a non-zero number of gas particles and $M_*>10^{10}M_\odot$, we find that $\sim 29.8\%$ (57/191) of galaxies at $z=0$ are gas poor based on \eq\ref{eq:massLowLim}.  This drops to $\sim 18.1\%$ (19/105) of galaxies at $z=1$ which are gas-poor, and $\sim 7.4\%$ (4/54) of galaxies at $z=2.$}
...
\begin{align}
    \textrm{Star Forming}:\quad &\ssfr\times \tH \geq 0.7\label{eq:ssfrt1}\\
    \textrm{Transitioning}:\quad &0.7>\ssfr\times \tH>0.3\label{eq:ssfrt2}\\
...
\begin{equation}
     ...
     \label{eq:ssfrt4}
\end{equation}
\red{Applying this to our sample shows that, among our initial population of galaxies with a non-zero number of gas particles and $M_*>10^{10}M_\odot$, at $z=0$ we find$\sim 57.1\%$ (109/191) of galaxies will also be `low star forming'.  At $z=1$, this drops to $\sim 33.3\%$ (35/105) of galaxies that will also be `low star forming', and at $z=2$ this again drops to $20.4\%$ (11/54) of galaxies.  This is line with the observational results from \cite{Weaver23}, in which they reported that massive galaxies at $z>1$ contain around $\sim20-30\%$ quiescent systems, and that this grows to around $50\%$ at $z<1$.}
...
\begin{figure}
    ...
    \includegraphics[width=\linewidth]{newFigures/fig6_ubvSpace_z0p5.png}
...
\begin{figure*}
    ...
    \includegraphics[width=\linewidth]{newFigures/fig7_sfr_IR_FUV_z0p5.png}
...
\begin{figure}
    ...
    % \vspace*{-.1in}
     ...
     \includegraphics[width=\linewidth]{newFigures/fig8_sfrIR_FUV_GD_GoldStandard_z0p5_newSelect.png}
...
\begin{figure}[h]
    ...
    \includegraphics[width=\linewidth]{newFigures/fig11_rejuvLabeled(1).png}
...
\begin{figure}[H]
    ...
    \includegraphics[width=\linewidth]{newFigures/fig12_imgsABCD.png}
...
\begin{figure}[H]
    ...
    \includegraphics[width=\linewidth]{newFigures/fig12_imgsEFGH.png}
...
\begin{figure}[H]
    ...
    \includegraphics[width=\linewidth]{newFigures/fig15_sfClass.png}
...
\begin{figure}[H]
    ...
    \includegraphics[width=\linewidth]{newFigures/fig16_massDist(1).png}
